...
const Cell = ({x, y}: CellProps) => {
  ...
  const [file, x_index, y_index,
    overlay_file, overlay_x_index, overlay_y_index] = useMemo(() => {
    switch (strToNature(tile?.nature as unknown as string)) {
      ...
      case TileNature.House:
        return [grass_map, 1, 0, houses_map, Math.floor(Math.random() * 3), Math.floor(Math.random() * 4)];
    }
  }, [tile]);
